export const movies = [
  {
    name: "The Prestige",
    imageLink:
      "https://assets.scriptslug.com/live/img/posters/the-prestige-2006.jpg",
    link: "https://www.imdb.com/title/tt0482571/?ref_=nv_sr_srsg_0_tt_7_nm_1_q_the%2520prestige",
  },
  {
    name: "The Dark Knight",
    imageLink:
      "https://m.media-amazon.com/images/M/MV5BMTMxNTMwODM0NF5BMl5BanBnXkFtZTcwODAyMTk2Mw@@._V1_.jpg",
    link: "https://www.imdb.com/title/tt0468569/?ref_=nv_sr_srsg_1_tt_7_nm_0_q_the%2520dar",
  },
  {
    name: "The Social Network",
    imageLink:
      "https://upload.wikimedia.org/wikipedia/en/8/8c/The_Social_Network_film_poster.png",
    link: "https://www.imdb.com/title/tt1285016/?ref_=nv_sr_srsg_0_tt_8_nm_0_q_the%2520social",
  },
];

export const directors = [
  {
    name: "Christopher Nolan",
    imageLink:
      "https://resizing.flixster.com/-XZAfHZM39UwaGJIFWKAE8fS0ak=/v3/t/assets/233377_v9_bb.jpg",
    link: "https://en.wikipedia.org/wiki/Christopher_Nolan",
  },
  {
    name: "David Fincher",
    imageLink:
      "https://images.mubicdn.net/images/cast_member/1469/cache-103636-1599486038/image-w856.jpg?size=800x",
    link: "https://en.wikipedia.org/wiki/David_Fincher",
  },
  {
    name: "Stanley Kubrick",
    imageLink:
      "https://media.gq.com/photos/5b4ba4e2b3161f214d1a06e3/16:9/w_2560%2Cc_limit/GettyImages-106952008.jpg",
    link: "https://en.wikipedia.org/wiki/Stanley_Kubrick",
  },
];

export const games = [
  {
    name: "Skyrim",
    imageLink:
      "https://image.api.playstation.com/vulcan/ap/rnd/202009/2820/h12URI7MdswtFPFHpkppNh2z.png",
    link: "https://en.wikipedia.org/wiki/The_Elder_Scrolls_V:_Skyrim",
  },
  {
    name: "Baldur's Gate 3",
    imageLink:
      "https://assets-prd.ignimgs.com/2023/08/24/baldursg3-1692894717196.jpeg",
    link: "https://en.wikipedia.org/wiki/Baldur%27s_Gate_3",
  },
  {
    name: "Factorio",
    imageLink:
      "https://upload.wikimedia.org/wikipedia/en/0/08/Factorio_cover.png",
    link: "https://en.wikipedia.org/wiki/Factorio",
  },
];

export const tvShows = [
  {
    name: "Mr Robot",
    imageLink:
      "https://images-na.ssl-images-amazon.com/images/S/pv-target-images/108517acee920b44a8679bf2dd0d9ec29f6728257513e62fd6b5610529185caf._RI_TTW_.jpg",
    link: "https://www.imdb.com/title/tt4158110/?ref_=nv_sr_srsg_0_tt_7_nm_1_q_mr%2520robot"
  },
  {
    name: "Breaking Bad",
    imageLink:
      "https://media.gq.com/photos/558288ab1177d66d68d51c9d/4:3/w_600,h_450,c_limit/blogs-the-feed-breaking-bad-roundup-635.jpg",
    link: "https://www.imdb.com/title/tt0903747/?ref_=nv_sr_srsg_0_tt_8_nm_0_q_breaki"
  },
  {
    name: "Dexter",
    imageLink:
      "https://m.media-amazon.com/images/M/MV5BZjkzMmU5MjMtODllZS00OTA5LTk2ZTEtNjdhYjZhMDA5ZTRhXkEyXkFqcGdeQXVyOTA3MTMyOTk@._V1_.jpg",
    link: "https://www.imdb.com/title/tt0773262/?ref_=nv_sr_srsg_0_tt_2_nm_6_q_dexter"
  },
];
